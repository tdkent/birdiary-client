export const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL as string;
export const SESSION_KEY = process.env.SESSION_KEY as string;
export const GOOGLE_API_KEY = process.env.NEXT_PUBLIC_GOOGLE_API_KEY as string;
export const GOOGLE_LOCATION_MAP_ID = process.env
  .NEXT_PUBLIC_GOOGLE_LOCATION_MAP_ID as string;
export const SUPPORT_EMAIL_ADDRESS = process.env
  .NEXT_PUBLIC_SUPPORT_EMAIL_ADDRESS as string;
export const TURNSTILE_SITE_KEY = process.env
  .NEXT_PUBLIC_TURNSTILE_SITE_KEY as string;
